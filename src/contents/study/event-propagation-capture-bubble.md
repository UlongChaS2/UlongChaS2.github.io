---
title: 'scroll 이벤트는 왜 window에서 잡히지 않을까'
date: '2026-08-07'
category: 'study'
keywords: ['JavaScript', 'Event Bubbling', 'Event Capturing', 'Event Delegation', 'React']
---

> 이벤트가 DOM 트리를 어떻게 지나가는지(캡처 → 타깃 → 버블), 왜 어떤 이벤트는 버블링하지 않는지, 그리고 버블링하지 않는 이벤트를 상위에서 잡으려면 왜 캡처여야 하는지 정리합니다.
...
이 한 줄이 동작하지 않는 이유를 이해하려면, 이벤트가 DOM 트리를 어떻게 지나가는지부터 봐야 합니다. (이 기능을 만든 전체 이야기는 [Shift 드래그 다중 선택 만들기](/project/shift-drag-multi-select)에 있습니다.)

## 이벤트는 세 단계로 전파된다

버튼을 클릭하면 이벤트는 버튼에서 태어나 버튼에서 끝날 것 같지만, 실제로는 문서 꼭대기부터 타깃까지 내려갔다가 다시 올라옵니다. DOM 표준은 이 경로를 세 단계로 정의합니다.
...
`addEventListener`의 세 번째 인자가 바로 "어느 방향에서 잡을 것인가"의 선택입니다. 기본값은 `false`(버블), `true` 또는 `{ capture: true }`를 주면 캡처 단계에서 잡습니다.
...
제 scroll 리스너가 동작하지 않은 이유가 여기에 있습니다. 이벤트마다 `bubbles`라는 속성이 있고, 이것이 `false`인 이벤트는 타깃에서 멈춥니다. 버블 단계 자체가 없습니다.
...
찾아보니 버그가 아니라 의도였습니다. 이유가 두 가지였습니다.
...
다시 처음 문제로 돌아옵니다. 캐시 무효화 리스너는 "문서 안 **어디서** 스크롤이 일어나든" 알아야 합니다. 스크롤되는 컨테이너가 무엇인지 미리 알 수 없으니 개별 등록도 어렵습니다. 버블링은 안 올라오는데, 그럼 방법이 없나 싶었습니다
...
- 이벤트는 캡처(내려감) → 타깃 → 버블(올라감) 세 단계로 전파되고, `addEventListener`의 `capture` 옵션은 어느 방향에서 잡을지의 선택입니다
...
window에 건 리스너가 조용하면 이제 `event.bubbles`부터 봅니다. 버블링하지 않는 이벤트라면 캡처로 잡으면 됩니다
