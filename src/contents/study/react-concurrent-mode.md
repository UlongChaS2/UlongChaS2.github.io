...
**Concurrent Mode**(동시성 모드)는 React 16.x/17에서 실험적으로 존재하다가 React 18에서 **Concurrent Features**로 정식화된 개념으로, **UI의 응답성을 향상**시키고, 여러 개의 업데이트를 보다 **부드럽고 유연하게 처리**하기 위한 새로운 렌더링 방식이다.

**기존 렌더링 방식과의 차이점**

> 참고: Fiber 아키텍처는 이미 React 16에서 도입됐고, 동기 렌더링과 동시성 렌더링은 **둘 다 Fiber 위에서 동작**한다. 따라서 대비는 "Fiber 이전 vs Concurrent"가 아니라 "동기 렌더링 vs 동시성 렌더링"이다.

- **동기(legacy) 렌더링**: 렌더링이 **동기적**(Synchronous)으로 진행됨 → 하나의 업데이트가 시작되면 그게 끝날 때까지 중단 없이 진행되어 다른 작업을 못 함.
- **동시성 렌더링(Concurrent)**: React가 업데이트를 **비동기적으로 처리**(Asynchronous)할 수 있도록 개선 → 중간에 렌더링을 **일시 중단**(pausing)하고, **우선순위**(priority)가 높은 작업을 먼저 실행할 수 있음.
...
### 3. 작업 분할(Time-Slicing)

React는 **자체 우선순위 스케줄러**(브라우저의 `requestIdleCallback`이 아니라 `MessageChannel` 기반)를 사용해서, 우선순위에 따라 렌더링 작업을 조금씩 쪼개고 중간중간 브라우저에 제어권을 양보(yield)함. 이를 **Time-Slicing**(시간 분할)이라고 함.
...
`useDeferredValue()`는 **어떤 값의 지연된 버전을 만들어서, 그 지연된 값으로 무거운 렌더링을 수행하게 함**. 덕분에 높은 우선순위 작업(예: 타이핑)을 방해하지 않음.
