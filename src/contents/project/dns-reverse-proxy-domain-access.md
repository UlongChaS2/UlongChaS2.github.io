...
이 문제를 겪으며 기술만큼 크게 남은 배움이 하나 더 있습니다. "인프라팀이 세팅했다는데 접속이 안 된다"는 상황이, 사실 **누가 일을 안 한 게 아니라 각자 자기 구간까지만 하고 그 사이에 빈 칸이 있던 것**이었다는 점입니다.
...
| 구간 | 하는 일 | 담당 |
|---|---|---|
| ① DNS | `app.example.co.kr = 172.16.10.5` 등록 | 인프라팀 |
| ② 네트워크/방화벽 | 사용자 PC → 서버 80 포트 통행 허용 | 인프라팀 |
| ③ 80 포트 응대 | 도착한 요청을 앱으로 안내 (리버스 프록시) | 개발자 |
| ④ 앱 | 실제 서비스 (8080) | 개발자 |

제 사례에서 ①②는 정상이었고 ③이 비어 있었습니다. 인프라팀은 자기 구간을 다 했기에 "세팅 완료"라고 말한 것이 맞았고, 정작 막힌 지점은 그들이 서버에 로그인해도 손댈 수 없는 서버 안쪽이었습니다.

경계를 가르는 기준은 한 줄로 정리됐습니다. **서버 바깥이냐, 서버 안이냐.**

- 서버 **바깥** (DNS 레코드, 방화벽 정책, 네트워크 경로) = 인프라팀. 서버에 들어가도 못 바꿉니다.
- 서버 **안** (nginx, 앱, 설정 파일, 배포) = 개발자. SSH로 들어가서 처리합니다.

이 구분은 OSI 7계층과도 상당히 겹칩니다. 물리에 가까운 아래층일수록 시설을 소유한 인프라팀, 소프트웨어에 가까운 위층일수록 그걸 만든 개발자 소관이라는 상식과 맞아떨어집니다.

![OSI 계층과 책임 경계 — 아래층은 인프라, 위층은 개발자](/images/dns-reverse-proxy-domain-access/osi-responsibility-boundary.svg)

다만 계층 번호만으로 담당을 정할 수 없는 예외도 있었습니다. **DNS는 응용 계층(L7)이지만 서버를 인프라팀이 소유하므로 인프라팀 담당**이고, **포트는 전송 계층(L4)이지만 "방화벽에서 80을 열어주는 것"은 인프라팀, "80에서 실제로 듣는 프로그램을 띄우는 것"은 개발자**로 양쪽이 관여합니다. 제가 겪은 문제가 정확히 이 포트 지점, 열려는 있는데 듣는 사람이 없던 상황이었습니다. 그래서 실무에서는 계층 번호보다 "서버 바깥이냐 안이냐"가 더 쓸모 있는 기준이라고 느꼈습니다.

경계를 알고 나니 인프라팀과의 소통도 짧아졌습니다. 근거와 요청 범위를 같이 적으면 한 번에 통했습니다.

- 우리 쪽이 원인일 때: "DNS와 네트워크는 정상 확인했고, 80 포트를 받아줄 프록시가 저희 서버에 없어서 생긴 문제입니다. 저희 쪽에서 설정 추가하겠습니다."
- 인프라팀에 넘길 때: "서버 내부에서는 앱이 정상 응답합니다. 사용자 PC에서 서버 80 포트로 접근이 안 되는 것 같은데 방화벽 정책 확인 부탁드립니다."

프론트 개발자가 왜 이런 것까지 알아야 하나 싶기도 했지만, 화면을 만드는 일도 결국 이 네 구간의 맨 끝에 얹혀 있습니다. 내 코드가 멀쩡한데 화면이 안 뜰 때, 문제를 구간으로 쪼개서 "여기까지는 정상, 여기서 끊겼다"를 말할 수 있는 사람과 "왜인지 모르겠어요"라고 하는 사람은 다르게 보입니다.
