...
여기서 두 가지를 알게 됐습니다. 하나는 hosts 파일이 DNS보다 우선이라는 것(그래서 진단할 때 hosts부터 봐야 합니다). 다른 하나는 내 PC는 전 세계 DNS를 뒤지는 게 아니라 설정된 DNS 서버 한 곳에만 물어본다는 것입니다. 이 두 번째 사실이 뒤에 나올 "같은 도메인이 위치마다 다른 곳을 가리키는" 현상의 열쇠였습니다.
...
이 문제를 겪으며 기술만큼 크게 남은 배움이 하나 더 있습니다. "인프라팀이 세팅했다는데 접속이 안 된다"는 상황이, 사실 누가 일을 안 한 게 아니라 각자 자기 구간까지만 하고 그 사이에 빈 칸이 있던 것이었다는 점입니다.
...
다만 계층 번호만으로 담당을 정할 수 없는 예외도 있었습니다. DNS는 응용 계층(L7)이지만 서버를 인프라팀이 소유하므로 인프라팀 담당이고, 포트는 전송 계층(L4)이지만 "방화벽에서 80을 열어주는 것"은 인프라팀, "80에서 실제로 듣는 프로그램을 띄우는 것"은 개발자로 양쪽이 관여합니다. 제가 겪은 문제가 정확히 이 포트 지점, 열려는 있는데 듣는 사람이 없던 상황이었습니다. 그래서 실무에서는 계층 번호보다 "서버 바깥이냐 안이냐"가 더 쓸모 있는 기준이라고 느꼈습니다.
