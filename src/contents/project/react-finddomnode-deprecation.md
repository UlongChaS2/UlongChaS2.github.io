...
`findDOMNode()`는 **컴포넌트의 실제 DOM 노드를 직접 가져오는 API**로, 예전에는 특정 컴포넌트가 실제로 렌더링된 DOM을 가져와야 할 때 유용했다. 그러나 React 16 이후로 점진적으로 사용이 제한되었고, React 18의 Strict Mode에서는 deprecation 경고가 뜨며(동작은 여전히 함), React 19에서 완전히 제거되었다.
...
### 3. Strict Mode에서 경고 대상

React 18의 **Strict Mode**에서는 `findDOMNode()`가 deprecation 경고 대상이다(동작 자체는 여전히 한다).
...
즉, **React 18의 Strict Mode에서는 `findDOMNode()` 사용 시 deprecation 경고가 표시되며, React 19에서는 이 API가 완전히 제거**되었다.

> 하지만 현실은 React 철학과 맞지 않을 때가 많다. 내 케이스는 recharts로 두 개의 stacked bar chart의 각 요소의 차이점을 실선과 라벨로 표현하는 것이었다. 결론적으로 나는 `findDOMNode()`를 사용하여 이 문제를 해결했는데, 왜 결국 저 많은 단점과 React 18 Strict Mode에서 경고까지 뜨는(React 19에서는 아예 사라진) `findDOMNode()`를 꼭 써야만 했는지, 사용하지 않으려고 어떤 시도를 했는데 결국 승복하고 말았는지에 대해서 설명한다.
...
- `ref`는 **자신이 직접 렌더링하는 host(DOM) 요소**에만 붙일 수 있음
- 그래서 **recharts처럼 내부에서 직접 요소를 생성하는 서드파티 컴포넌트의 개별 요소(bar)에는 직접 접근할 수 없음**
