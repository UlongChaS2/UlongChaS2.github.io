...
이벤트가 왜 세 단계로 여행하는지, 어떤 이벤트가 왜 버블링하지 않는지는 별도 글에서 깊게 다뤘습니다: [scroll 이벤트는 왜 window에서 잡히지 않을까](/study/event-propagation-capture-bubble)
...
코드가 버그 수정 전보다 오히려 50줄 줄었습니다. 모델을 바꾸니 예외 처리가 필요 없어졌습니다.
...
- 공유 요소에 리스너를 걸 때는 **"이 컴포넌트가 동시에 몇 개 마운트되나"**를 먼저 확인하게 됐습니다 - 접힌 아코디언, 탭, 가상 리스트에는 안 보이는 인스턴스가 살아 있습니다
- 가시성 판정에 `getBoundingClientRect().height`는 쓸 수 없었습니다 - **overflow로 잘린 요소는 원래 크기를 반환합니다.** `offsetHeight`(조상 포함)나 `checkVisibility()`로 바꿨습니다
